...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let x = 0; x <= W; x += 40) {
      grassG.moveTo(x, 0); grassG.lineTo(x, H);
    }
    for (let y = 0; y <= H; y += 40) {
      grassG.moveTo(0, y); grassG.lineTo(W, y);
    }
    grassG.strokePath();

    // Center circle (park vibe)
    ...
    // Scattered paw prints for decoration
    for (let i = 0; i < 6; i++) {
    ...
    // Bone shape: two circles + rect
    const boneL = this.add.circle(-8, 0, 7, 0xfff8dc); // left knob
    const boneR = this.add.circle(8, 0, 7, 0xfff8dc);  // right knob
    const boneM = this.add.rectangle(0, 0, 16, 8, 0xfff8dc).setOrigin(0.5);
    const boneHighlight = this.add.circle(-4, -3, 3, 0xffffff, 0.4);
    // Star sparkle
    ...
    this.boneGraphic.add([boneL, boneR, boneM, boneHighlight, sparkle]);
    this.boneGraphic.setVisible(false);
    this.boneGlow.setVisible(false);

    // --- Scoreboard (styled panel) ---
    ...
    this.scoreboard = this.add.text(14, 12, '', {
      fontSize: '12px', color: '#ffffff', fontFamily: 'Jua, Consolas, monospace',
      lineSpacing: 3,
    });
  ...
  _createPlayerGraphic(p) {
    ...
    const nameTextObj = this.add.text(0, 0, p.name, {
      fontSize: '11px', color: '#ffffff', fontFamily: 'Jua, sans-serif',
      stroke: '#000000', strokeThickness: 2,
    }).setOrigin(0.5);
    nameTag.add(nameTextObj);
  ...
  _updatePlayerGraphic(p) {
    ...
    // Body color states
    const body = g.container.list[4]; // 5th element = body circle
    if (p.isDashing) {
      ...
      if (p.hasBone) {
        body.setFillStyle(0xffe070);
      } else {
        body.setFillStyle(0xf5deb3);
      }
    }
  }
  ...
  _onGameEnd(data) {
    ...
    const btnText = this.add.text(0, 0, '\ub85c\ube44\ub85c \ub3cc\uc544\uac00\uae30', {
  ...
  _spawnConfetti(W, H) {
    const confettiColors = [0xffd700, 0xff6b6b, 0x5dade2, 0x2ecc71, 0xf39c12, 0x9b59b6];
    for (let i = 0; i < 40; i++) {
      const x = Phaser.Math.Between(50, W - 50);
      const size = Phaser.Math.Between(4, 8);
      const c = Phaser.Math.RND.pick(confettiColors);
